...
static void	check_for_dups(struct s_node *curr, int nb, struct s_node *stack_a)
{
	curr = curr->prev;
	while (curr != NULL)
	{
...
static size_t	if_space_or_sign(const char *nptr, struct s_node *stack_a)
{
	size_t	i;
...
static int	atoi_err(const char *nptr, struct s_node *stack_a)
{
	size_t		i;
	int			minus;
	long long	result;
